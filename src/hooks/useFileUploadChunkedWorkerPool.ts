 ...
 *   4. Send the remaining chunks to the server CONCURRENTLY, using a small
 *      pool of workers sized to the detected connection quality.
 *
...
const useFileUploadChunkedWorkerPool = () => {
  ...
  // ---------------------------------------------------------------------
  // Step 3 — send the chunks that are still left, using a small pool of
  // concurrent workers sized to the detected connection quality.
  //
  ...
  // two workers can never claim the same index). This reads directly from
  // session.remainingChunks, so it works both for a brand new upload
  // (handleUpload) and for resuming a paused one (handleResume) — both just
  ...
  const sendRemainingChunks = async (id: string) => {
    ...
    // All workers finished without pausing or failing — the file is fully
